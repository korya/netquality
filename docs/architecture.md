...
| The engine is pure | `internal/engine` sees only `Observation`s (elapsed, bytes, flows, probe samples) and returns `Decision`s; it holds no clock, goroutine, or socket, so the same code runs against real transports and, in tests, against recorded series or a simulator. The public latency, confidence and stability-parameter types are declared in the library package and converted at the seam, so that pkg.go.dev documents them and a refactor inside the engine cannot change the public API by accident. The interval loop and discovery/idle/load timeout creation in `run.go` go through the injectable clock; production deadlines use `context.WithTimeout`. |
...
| Server budgets gate admission | One mutex protects per-identity byte credit and active transfer slots; streaming runs outside that mutex. Accounting is per Handler instance and covers application payload. Active and indebted identities survive cleanup. Config/small requests do not consume transfer slots. No background scheduler or request queue. |
